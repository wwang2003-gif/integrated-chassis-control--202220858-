...
if ~isfield(ctrlState, 'intError')
    ...
    Fx_cmd = CTRL.LON.Kp * error_vx + CTRL.LON.Ki * ctrlState.intError;

    if error_vx < -5.0 
        Fx_cmd = -1e6; 
    end
    
    % 2.  코디네이터가 4륜 독립 제어를 할 수 있도록 슬립 배열 전체를 넘김
    if isfield(ctrlState, 'wheelSlip')
        forceCmd.wheelSlip = ctrlState.wheelSlip;
    else
        forceCmd.wheelSlip = zeros(4,1);
    end
    forceCmd.ax = ax; 
